...
file_n:				# file name (0 terminated)
	.string	"testfile.txt"	#na końcu dodane będzie \0 - zakonczenie strnig 
...
txtlen:				# size of written data
	.quad		( . - txtline )			#.(kropka) - bieżaca adres w programie, zamiast może być txtlen 
...
loop:	
	...
	CMP	%rdx,%rax
	JNZ	error		# if RAX<>RDX then something went wrong #JNZ- jump if no zero
